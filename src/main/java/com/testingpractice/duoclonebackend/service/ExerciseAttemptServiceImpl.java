...
public class ExerciseAttemptServiceImpl implements ExerciseAttemptService {
  ...
  public List<ExerciseAttempt> getLessonExerciseAttemptsForUser(Integer lessonId, Integer userId) {
    ...
    return exerciseAttemptRepository.findAllByExerciseIdInAndUserIdAndUnchecked(
        exerciseIds, userId);
  }
  ...
  private ExerciseAttemptResponse getExerciseAttemptResponseMessage(
    ...
    ArrayList<Integer> correctResponses =
        exerciseOptionService.getCorrectExerciseResponses(optionIds, correctOptions);
    String correctAnswer = exerciseOptionService.getCorrectExerciseAnswerText(correctOptions);

    if (areCorrect) {
      return new ExerciseAttemptResponse(
          true, attempt.getScore(), "Correct!", correctResponses, correctAnswer);
    } else {
      return new ExerciseAttemptResponse(
          false, attempt.getScore(), "Incorrect!", correctResponses, correctAnswer);
    }
  }

  private List<AttemptOption> getAttemptOptions(
      ExerciseAttempt attempt, ArrayList<Integer> optionIds) {
